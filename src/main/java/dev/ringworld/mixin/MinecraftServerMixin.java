...
abstract class MinecraftServerMixin implements RingWorldStorageAccess {
     ...
     * can cross either periodic edge. Canonicalize the final spawn position,
     * not just the original sampler suggestion, before it reaches saved level
     * data.
     */
    @Redirect(
            method = "setInitialSpawn",
    ...
    private static void ringworld$canonicalInitialSpawn(
            ServerLevelData levelData, BlockPos position, float angle) {
        RingWorldConfig config = RingWorldConfig.load();
        RingGeometry geometry = new RingGeometry(config.widthBlocks(), config.circumferenceBlocks());
