...
palettes=[('weathered','Weathered stone'),('ancient','Ancient masonry'),('natural','Natural rock'),('alloy','Ring alloy'),('industrial','RingWorld'),('overgrown','Overgrown ruin'),('monolith','Clean monolith'),('nether','Nether fortress'),('obsidian','Obsidian bastion'),('wood','Timber rampart')]
patterns=[('engineered','RingWorld Structure',6),('masonry','Masonry',1),('gradient','Gradient',4)]
...
def label(im,text,x,y,scale=2):
    for c in text:
        ...
        x+=(width+1)*scale
entries=[]; sheet=Image.new('RGB',(1440,3260),'#141c24')
...
manifest=dict(schemaVersion=1,minecraftVersion='26.2',loader='Fabric',sourceCommit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
    ...
    'Source center chosen to show the generated exposed-machinery motif with RingWorld / RingWorld Structure.',
    'RingWorld Structure adds geometric elements only with the RingWorld palette.',
    'Clustered, Strata, Panels and Hybrid remain readable in older saves but are not selectable and are excluded.',
...
<main><h1>Wall material × pattern</h1><p>30 real Minecraft samples. Matched camera, noon, clear weather, 7-block thickness and 0% decay.</p>
<label>Material <select id="material"></select></label><label>Pattern <select id="pattern"></select></label>
<img id="hero" alt="Selected wall sample"><p id="caption"></p><p><a id="raw">Clean full-size screenshot</a><a id="preview">Selector preview</a><a href="comparison.jpg">All 30 together</a><a href="manifest.json">Asset manifest</a></p>
<p>RingWorld Structure adds machinery only with the RingWorld material. These isolated specimens use the actual generation functions; terrain and weather will affect their appearance in a world.</p><div id="grid"></div></main><script>const entries=DATA;
...
const selected=entries.find(e=>'#'+e.id===location.hash)||entries.find(e=>e.palette==='INDUSTRIAL'&&e.pattern==='ENGINEERED');if(selected){material.value=selected.palette;pattern.value=selected.pattern}show();</script></html>'''
