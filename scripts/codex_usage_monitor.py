...
def find_codex_binary(explicit: Optional[str] = None) -> str:
    ...
    raise MonitorError(
        "Codex executable not found; set CODEX_BIN or pass --codex-bin."
    )
...
def read_response(
    ...
    try:
        while True:
            ...
            if remaining <= 0:
                raise MonitorError(
                    "Timed out waiting for the Codex usage response."
                )
            ...
            if not line:
                raise MonitorError(
                    "Codex app-server ended before returning usage data."
                )
            try:
...
def query_rate_limits(
    codex_binary: str, timeout_seconds: float = 30.0
) -> Dict[str, Any]:
    environment = os.environ.copy()
    # This monitor is specifically for the signed-in ChatGPT allowance. Do not
    # let an unrelated developer API key choose a different auth path.
    for variable in (
        "OPENAI_API_KEY",
    ...
    try:
        send_message(
            process,
            {
                ...
                "params": {
                    "clientInfo": {
                        "name": "ringworld_usage_monitor",
                        "title": "RingWorld Usage Monitor",
                        "version": "1.0.0",
                    }
                },
            },
...
def select_weekly_window(
    result: Dict[str, Any],
    weekly_window_minutes: int = WEEKLY_WINDOW_MINUTES,
) -> Dict[str, Any]:
    ...
    if not weekly:
        ...
        raise MonitorError(
            "No {}-minute weekly quota window was returned; available window "
            "durations: {}.".format(weekly_window_minutes, available)
        )
    # If the service returns multiple weekly buckets, protect the most depleted one.
...
def install_launch_agent(args: argparse.Namespace) -> int:
    ...
    status_path = runtime_dir / "codex-usage-status.json"
    ...
    plist = {
        ...
        "StandardOutPath": str(runtime_dir / "codex-usage-monitor.out.log"),
        "StandardErrorPath": str(runtime_dir / "codex-usage-monitor.err.log"),
    }
...
def uninstall_launch_agent() -> int:
    ...
    try:
        ...
        pass
    try:
        (application_support_path() / "codex_usage_monitor.py").unlink()
    except FileNotFoundError:
        pass
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Read the ChatGPT-backed Codex weekly quota through the supported "
            "local app-server account endpoint."
        )
    )
    parser.add_argument("--codex-bin", help="Explicit Codex executable path.")
    parser.add_argument(
        "--status-file",
        default=str(repo_root / ".codex-tmp/codex-usage-status.json"),
    ...
    parser.add_argument(
        "--weekly-window-minutes",
        type=int,
        default=WEEKLY_WINDOW_MINUTES,
        help="Expected weekly quota window duration.",
    )
    parser.add_argument(
        "--timeout", type=float, default=30.0, help="App-server response timeout."
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    actions.add_argument(
        ...
        action="store_true",
        help="Install the five-minute macOS background monitor.",
    )
    actions.add_argument(
