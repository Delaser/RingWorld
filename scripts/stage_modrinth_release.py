...
SENSITIVE_BASENAMES = {".env", "accounts.json", "eula.txt", "level.dat", "ops.json", "options.txt", "servers.dat", "server.properties", "session.lock", "usercache.json", "whitelist.json"}
...
def validate_archive_paths(names: list[str]) -> None:
    ...
    for name in names:
        ...
        if basename in SENSITIVE_BASENAMES or (lowered_parts and lowered_parts[0] in SENSITIVE_TOP_LEVEL):
...
def stage_release(jar_path: Path, config_path: Path, description_path: Path, changelog_path: Path, license_path: Path, output_root: Path) -> Path:
    ...
    try:
        ...
        manifest = {
            ...
            "source": config["source"], "publication_action": "manual_owner_authorization_required"
        }
...
def main() -> int:
    ...
    try:
        ...
        target = stage_release(jar, args.config, args.description, args.changelog, args.license, args.output_root)
