...
from minecraft_world_upgrade_qualification import FIXTURE_NAME, SUPPORTED_FORWARD_PATHS
...
def run(
    ...
    source_cell = _exact_cell(manifest, arguments.source_cell)
    ...
    if source_cell.get("loader") != target_cell.get("loader") \
            or (source_version, target_version) not in SUPPORTED_FORWARD_PATHS:
    ...
    source = ForwardUpgradeSource(
        ...
        source_terminal, terminal_hash, source_world, source_resume_log, resume_hash,
    )
    return executor(
        ...
        range_identities=reviewed_range_identities(), candidate_inspector=candidate_inspector,
