...
def release_config(loader: str = "fabric") -> dict:
    public_version = f"1.0.0-{loader}+mc26.1.2"
    ...
    config = {
        ...
        "version": {"name": f"RingWorld 1.0 for Minecraft 26.1.2 ({public_loader})", "version_number": public_version, "artifact_version": VERSION, "version_type": "release", "game_versions": ["26.1.2"], "loaders": [loader], "environment": "client_and_server", "dependencies": [{"project_id": "P7dR8mSH", "dependency_type": "required"}] if loader == "fabric" else [], "featured": True},
...
def write_jar(path: Path, *, minecraft: str = "26.1.2", identifier: str = "MPL-2.0", environment: str = "*", compatibility_api: int = 1, sensitive: str | None = None, sensitive_content: str = "secret", embedded_license: bytes = LICENSE, contract_mutation: str | None = None, build_identity: bytes | None = b"artifactVersion=1.0.0+mc26.1.2\nreleaseLabel=1.0\n") -> None:
...
def write_neoforge_jar(path: Path, *, minecraft: str = "26.1.2", neoforge: str = "[26.1.2.87,)", identifier: str = "MPL-2.0", mod_id: str = "ringworld", embedded_license: bytes = LICENSE, contract_mutation: str | None = None, build_identity: bytes | None = b"artifactVersion=1.0.0+mc26.1.2\nreleaseLabel=1.0\n") -> None:
...
class ModrinthStagingTest(unittest.TestCase):
    ...
    def test_rejects_ambiguous_or_wrong_loader_public_version(self) -> None:
        for loader, public_version in (
                ("fabric", VERSION),
                ("fabric", "1.0.0-neoforge+mc26.1.2"),
                ("neoforge", "1.0.0-fabric+mc26.1.2")):
    ...
    def test_rejects_missing_or_mismatched_artifact_version(self) -> None:
        for artifact_version in (None, "", "1.0.1+mc26.1.2"):
        ...
        for identity in (None,
                         b"artifactVersion=1.0.1+mc26.1.2\nreleaseLabel=1.0\n",
                         b"artifactVersion=1.0.0+mc26.1.2\nreleaseLabel=Alpha 4\n"):
    ...
    def test_allows_normal_release_versions_and_non_revision_hex_word(self) -> None:
        write_jar(self.jar)
        self.description.write_text(
            "RingWorld 1.0.0+mc26.1.2 requires Java 25; its defaced cobblestone rim "
