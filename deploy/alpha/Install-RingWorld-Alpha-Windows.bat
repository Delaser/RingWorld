...
set "MANIFEST_PATH=%TEMP%\RingWorld-Alpha-RELEASE-MANIFEST.json"
set "ZIP_PATH=%TEMP%\RingWorld-Alpha-Windows.zip"
set "INSTALL_DIR=%LOCALAPPDATA%\RingWorld\Alpha4"

echo Checking for the newest RingWorld alpha build...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop';" ^
  "$ProgressPreference='SilentlyContinue';" ^
  "[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12;" ^
  "$headers=@{'Cache-Control'='no-cache'};" ^
  "$manifestUrl=$env:BASE_URL+'/RELEASE-MANIFEST.json';" ^
...
echo Package verified. Starting the RingWorld launcher...
...
del /Q "%ZIP_PATH%" "%MANIFEST_PATH%" 2>nul
echo.
echo RingWorld installation failed. No unverified package was launched.
