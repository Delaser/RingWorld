...
| 13 | `NoiseChunkGeneratorMixin` | `NoiseChunkGenerator.populateNoise`, `buildSurface`, `carve`, broad `method="*"` router redirect, `createChunkNoiseSampler` | `net.minecraft.world.level.levelgen.NoiseBasedChunkGenerator.fillFromNoise`, `buildSurface`, `applyCarvers`, `RandomState.router()` call sites, private `createNoiseChunk` | Low | The old broad redirect is unsafe: 26.1 has multiple `router()` and `createNoiseChunk` sites. Do not use `method="*"` until each inspected call is classified. | Compile/mixin audit, multi-seed seam density/surface/carver fixtures, aquifers, exterior/rims, Nether/End. |
