...
| 11 | `MinecraftServerMixin` | `MinecraftServer.setupSpawn`, redirect `findBestSpawnPosition` | `net.minecraft.server.MinecraftServer.setInitialSpawn(...)`; inspected private static spawn setup; redirects the sampler and every `RespawnData.of` result; protected `storageSource` bridge to `LevelStorageAccess.getDimensionPath` | Medium | The method is static/private and has a `LevelLoadListener`; constrain only the Overworld spawn search, canonicalize every final saved spawn after vanilla's seam-crossing safe-spawn spiral, and retain progress reporting. The storage bridge is read-only so callers cannot bypass Minecraft's path ownership. | Fresh safe-small and production creation, negative/seam-alias final spawn canonicalization, spawn interior bounds, ordinary debug world, Overworld dimension-path assertion. |
...
| 13 | `NoiseChunkGeneratorMixin` | `NoiseChunkGenerator.populateNoise`, `buildSurface`, `carve`, broad `method="*"` router redirect, `createChunkNoiseSampler` | `net.minecraft.world.level.levelgen.NoiseBasedChunkGenerator.fillFromNoise`, `buildSurface`, `applyCarvers`, exact `doCreateBiomes` router call, private `createNoiseChunk`, and private `iterateNoiseColumn` | Medium | Source audit narrowed the former wildcard: `doCreateBiomes` is the direct gameplay router consumer; terrain density and the shared base-height/base-column path remain scoped through `createNoiseChunk`/`iterateNoiseColumn` and `NoiseChunk.<init>`. The private query canonicalizes X at its header before vanilla cell/cache interpolation, then enters the periodic router context; Z and null-geometry generators stay vanilla. Debug-screen router access is deliberately vanilla. The private factory and constructor ABI remain runtime-sensitive. | Compile/mixin audit; real-server periodic base-height/base-column aliases plus canonical noise-complete `WORLD_SURFACE_WG` agreement; multi-seed seam density/biome/surface/carver fixtures, aquifers, exterior/rims, Nether/End. |
...
| 22 | `ServerWorldMixin` | `ServerWorld` constructor, loaded/load/tick checks, synthetic `method_31420`, proximity delivery | `net.minecraft.server.level.ServerLevel`; constructor plus the private synthetic `lambda$tick$0(TickRateManager,ProfilerFiller,Entity)` consumer redirect of `DistanceManager.inEntityTickingRange`, `areEntitiesLoaded`, `waitForEntities`, tick/spawn predicates, and final `sendParticles` proximity path | Medium | The first 26.1 server gate proved that the entity eligibility call is not in `tick` bytecode: `tick` passes the synthetic consumer to `EntityTickList.forEach`. The exact synthetic descriptor is version-pinned; no fallback may become global forced ticking. | Required-injection server launch, moving entity eligibility, scheduled ticks, spawn checks, proximity particles/effects, save/reconnect, Nether/End. |
...
| 26 | `CreateWorldScreenMixin` | `CreateWorldScreen.init` tail | `net.minecraft.client.gui.screens.worldselection.CreateWorldScreen.init()`, redirecting its single `HeaderAndFooterLayout.addToFooter(LayoutElement)` call | High | The 26.1 screen uses a managed footer and reuses the parent instance after the editor closes. Add the entry to that footer row, refresh its C×W message through the UI-local parent hook without reinitializing the screen, keep compact-height layout, and preserve the single background/blur owner. | GUI scale 4 multi-frame check at 480×270 logical pixels; non-overlapping footer; invalid, safe-small, production, and custom validation/cost/confirmation matrix; accepted custom C×W summary refresh. |
