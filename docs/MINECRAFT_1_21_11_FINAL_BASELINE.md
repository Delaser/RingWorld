...
The active server reported zero players before backup. It was flushed through
RCON, automatic saving was paused for the archive, and saving was restored.

```text
/opt/ringworld-server/backups/mc-1.21.11-final-20260728T082713Z
```

The protected 60 MiB backup contains the active server launcher, RingWorld and
Fabric jars, configuration, checksums, world, `meridian-proximity` datapack,
both published client packages, RingWorld web page, and a Git archive of
deployment templates and operating documentation.

This backup must not be copied into the repository or a shareable client
bundle.
