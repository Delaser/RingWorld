...
| `StrongholdStructureMixin` | `StrongholdStructure.generatePieces` | Minimally translates the completed terrain-adjusted guaranteed-stronghold graph into canonical X and finite Z; if a 128-wide band cannot contain optional branches, fits the inflated portal-room bounds instead | Omitting the 12-block terrain-adjustment envelope can leave the required portal room outside the band; rejecting an oversized optional graph crashes narrow-world chunk generation; applying it without saved policy mutates legacy layouts |
...
| `BlockEntityMixin` | `BlockEntity.getPosFromTag` | Retains a raw periodic-X alias only during guarded RingWorld chunk post-load so collision recovery sees both saved NBT entries | Applying outside the scoped Overworld load context changes vanilla wrong-chunk repair; collapsing early can discard one old inventory |
| `BlockEntityPositionAccessor` | `BlockEntity` | Reassigns a newly registered server block entity to its canonical owner before vanilla inserts it into the chunk map | Use only from the guarded `LevelChunkMixin`; changing client presentation positions corrupts curved block-entity rendering |
| `LevelChunkMixin` | `LevelChunk` | Canonicalizes server-Overworld block-state and block-entity map positions at runtime | Must never affect client presentation chunks; old duplicate alias inventories require lossless recovery rather than silent deletion |
...
| `NoiseChunkGeneratorMixin` | `NoiseBasedChunkGenerator` | Attaches the persisted mapping, skips exterior work, scopes periodic biome/density/height/surface sampling, canonicalizes height-query X, and gives mapping-3+ carvers one periodic seed identity | Mapping selection must remain paired across every path; changing an existing world's mapping creates mixed terrain |
...
| `PortalForcerMixin` | `PortalForcer.findClosestPortalPosition`, `createPortal` | Normalizes already-scaled Overworld portal targets to canonical X and a frame-safe finite Z anchor; unions three periodic POI query images and returns one canonical safe portal | Applying after lookup can create exterior/alias portals; applying in Nether/End changes vanilla dimensions; a flat single-image search misses a linked portal across X=0/C |
...
| `RaidFixtureAccessor` | `Raid` | Exposes only pre-wave cooldown, group count, and bossbar state to the opt-in two-process seam-raid regression | Production code must not use it to skip raid waves or mutate raid state |
...
| `SurfaceNoiseThresholdMixin` | generated surface-rule noise condition | Maps mapping-3 threshold noise through the annular identity | Applying to mapping 1/2 changes existing terrain |
| `SurfaceSystemMixin` | `SurfaceSystem` | Maps mapping-3 surface depth/secondary, clay, badlands, frozen-ocean, and random coordinates | Scaled noises must map the original intrinsic block coordinate before reapplying vanilla scale |
...
| `CompassAngleStateFixtureAccessor` | `CompassAngleState.calculate` | Invokes the real mixed compass property only for the opt-in dual-loader map/compass acceptance fixture | Test-only bridge; production code must not call it or duplicate compass behavior outside `CompassAngleStateMixin` |
...
| `ClientConnectionMixin` | `Connection` | Canonicalizes every audited outbound positional block packet: break/use, sign, pick-block, block-entity query, command block, structure block, jigsaw edit/generate, test block, and test-instance action; block-use translates the clicked block and hit vector by one shared chart offset | Missing a packet type makes visible seam blocks non-interactive or edits the wrong canonical block; independently wrapping a seam-face hit makes vanilla reject it as distant |
...
| `GlobalSettingsMixin` | `GlobalSettingsUniform.<init>` and `update(..., Vec3, ...)` | Extends Globals with layout, vertical, render-distance, handoff, detail/reveal, haze, cloud-fade, visual-profile, and inner cloud-Z planes | Shader ABI extension; std140 order and buffer size are version-sensitive |
| `GuiMixin` | `Gui.extractRenderState` | Draws the small top-left `Ring Atlas Generating: X%` label while the acknowledged client Atlas is incomplete | Obeys Hide GUI, RingWorld-only state, and completion; never infer server job state or remain visible at 100% |
