...
| `NoiseChunkGeneratorMixin` | `NoiseBasedChunkGenerator` | Attaches geometry, skips exterior density/surface/carvers, scopes the periodic router to biome, real-terrain, and shared base-height/base-column sampler construction | The private sampler factory, `iterateNoiseColumn` constructor redirect, and biome climate call must remain paired without intercepting unrelated router consumers; a missed query sampler floats heightmap-projected structures |
