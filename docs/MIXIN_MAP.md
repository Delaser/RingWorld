...
| `NoiseChunkGeneratorMixin` | `NoiseBasedChunkGenerator` | Attaches geometry plus the persisted terrain-noise mapping, skips exterior density/surface/carvers, scopes the versioned periodic router to biome, real-terrain, and shared base-height/base-column sampler construction, and canonicalizes query X before private height-query interpolation | Mapping selection and cache invalidation must remain paired across real terrain, climate, and `iterateNoiseColumn`; changing a legacy world to annular creates mixed terrain, while a raw alias or missed query sampler floats heightmap-projected structures |
