...
The port was initially developed by two coding agents on separate dedicated
PCs and separate Git clones. GitHub issue
...
[#5–#13](https://github.com/Delaser/RingWorld/issues). The task history records
the original ownership, dependencies, assignments, and integration order.
...
- archive the active server jar, client packages, checksums, configuration,
  world, and deployment documentation outside this source repository;
...
2. retain the complete 1.21.11 service, world, jar, config, and packages;
3. upgrade a copied production world;
4. install the 26.1.2 server and Fabric API;
5. inspect startup and migration logs;
6. join with two packaged clients;
7. rerun seam, combat, block, atlas, and reconnect checks;
8. publish clearly versioned packages;
9. retain immediate rollback.
